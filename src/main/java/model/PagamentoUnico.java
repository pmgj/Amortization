...
public class PagamentoUnico extends Amortizacao {
    ...
    public void capital(Emprestimo loan) {
        ...
        for (int n = 0; n < loan.getTempo(); n++) {
            ...
            System.out.println(String.format("%2d %15s %15s %15s %15s", n, formatter.format(juros), formatter.format(0.0), formatter.format(0.0), formatter.format(saldo)));
            /* Calculando valores */
            juros = saldo * loan.getTaxa();
            saldo += juros;
            somaJuros += juros;
        }
        System.out.println(String.format("%2d %15s %15s %15s %15s", loan.getTempo(), formatter.format(juros), formatter.format(loan.getMontante()), formatter.format(saldo), formatter.format(0.0)));
