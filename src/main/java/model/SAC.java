...
public class SAC extends Amortizacao {
    ...
    public void capital(Emprestimo loan) {
        ...
        double juros = 0, amortizacao = 0, saldo = loan.getMontante(), somaJuros = 0;
        NumberFormat formatter = NumberFormat.getCurrencyInstance();
        for (int n = 0; n < loan.getTempo(); n++) {
            /* Preenchendo a tabela */
            System.out.println(String.format("%2d %15s %15s %15s %15s", n, formatter.format(juros), formatter.format(amortizacao), formatter.format(juros + amortizacao), formatter.format(saldo)));
            /* Calculando valores */
        ...
        System.out.println(String.format("%2d %15s %15s %15s %15s", loan.getTempo(), formatter.format(juros), formatter.format(amortizacao), formatter.format(juros + amortizacao), formatter.format(0.0)));
